...
// Function to encrypt plaintext using CBC mode with S-DES
void cbc_encrypt(const unsigned char* plaintext, unsigned char* ciphertext, const unsigned char* key, const unsigned char* iv, int length) {
    ...
    for (int i = 0; i < length; i++) {
        unsigned char plaintext_block[1] = {plaintext[i] ^ prev_block[0]};
        sdes_encrypt(plaintext_block, &ciphertext[i], K1, K2);
        prev_block[0] = ciphertext[i];
    }
}

// Function to decrypt ciphertext using CBC mode with S-DES
void cbc_decrypt(const unsigned char* ciphertext, unsigned char* plaintext, const unsigned char* key, const unsigned char* iv, int length) {
    ...
    for (int i = 0; i < length; i++) {
        unsigned char decrypted_block[1] = {0};
        sdes_decrypt(&ciphertext[i], decrypted_block, K1, K2);
        plaintext[i] = decrypted_block[0] ^ prev_block[0];
        prev_block[0] = ciphertext[i];
    }
}

int main() {
    unsigned char iv[1] = {0xAA};  // Binary: 1010 1010
    unsigned char plaintext[2] = {0x01, 0x23};  // Binary: 0000 0001 0010 0011
    unsigned char key[2] = {0x7D};  // Binary: 01111 11101

    unsigned char ciphertext[2] = {0};
    unsigned char decryptedtext[2] = {0};
    ...
    print_binary("Decrypted text", decryptedtext, 2);
